...
version 7.0;
syntax [newvarlist] [if] [in] [, FRom(varlist min=1 string) STring DOfile(string) PArse(string) ];
*
...
if "`varlist'"=="" {;
  ...
  * Create value file for each variable of -from- *;
  ...
  while `i1'<`nfrom' {;local i1=`i1'+1;
    ...
    strsep `fromcur',lhs(`lhs') rhs(`rhs') sep(`"`parse'"');
    tempfile tf`i1';
    valfile `lhs' `tf`i1'';
    drop `lhs' `rhs';
  };
  * Merge value files *;  
  ...
  while `i1'<`nfrom' {;local i1=`i1'+1;
    if `i1'==1 {;
      ...
      merge `lhs' using `tf`i1'',_merge(`mrg');
      drop `mrg';
      sort `lhs';
    };
  };
